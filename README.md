...
Собираем nginx 1.23.3 c поддержкой tls v1.3. (openssl-1.1.1q - 2022-Oct-12) Описание используемых параметров:
...
Скрипт для установки - script.sh
```bash
#!/bin/bash
sudo su
yum install -y \
...
Vagrant.configure(2) do |config|
  ...
  config.vm.define "otus7" do |srv|
    srv.vm.network "private_network", ip: "192.168.56.10"
    srv.vm.hostname = "otus7"
...
```bash
[vagrant@otus6 ~]$ cat /root/rpmbuild/SPECS/nginx.spec
cat: /root/rpmbuild/SPECS/nginx.spec: Отказано в доступе
[vagrant@otus6 ~]$ sudo cat /root/rpmbuild/SPECS/nginx.spec
#
...
```bash
[root@otus6 yum.repos.d]# yum repolist enabled | grep custom
custom                              custom-repo                                1
[root@otus6 yum.repos.d]# yum list --showduplicates | grep custom
gnome-session-custom-session.x86_64      3.28.1-8.el7                  base     
nginx.x86_64                             1:1.23.3-1.el7.ngx            custom  
